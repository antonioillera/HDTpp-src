...
void BasicHDT::mapHDT(const char *fileNameChar, ProgressListener *listener) {
    ...
    StopWatch st;
            TripleID t=TripleID(0,0,0);
                IteratorTripleID *it = this->getTriples()->search(t);
                while (it->hasNext()) {
                        TripleID *tid = it->next();
                unsigned int s = tid->getSubject();
                        unsigned int p = tid->getPredicate();
                unsigned int o = tid->getObject();
            }
             cout << "Tiempo de recorrido: " << st << endl;
}


size_t BasicHDT::loadMMap(unsigned char *ptr, unsigned char *ptrMax, ProgressListener *listener) {
	cout<<"Carga HDT Original y genera familia"<<endl;
	size_t count=0;
    ControlInformation controlInformation;
    IntermediateListener iListener(listener);
